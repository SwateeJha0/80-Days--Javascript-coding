...
const colorObj = {
    ...
    color4: {current: "#006400" , new:"#ffff00"},
};
...
//function to get a random color from colors objects
const getRandomColor = (colorObj) => {
    const colorKeys = Object.keys(colorObj);
    return colorKeys[Math.floor(Math.random() * colorKeys.length)];
};

//function to pause exceution of game for given amount of time 
...
//function to show the path of colors to players 
...
const showPath = async(colors) => {
    ...
    //pause exceution for 500 miliseconds
        await delay(500);
